// Official brand glyphs (single-path, 24x24) from the simple-icons set.

type Props = { className?: string; style?: React.CSSProperties };

export function InstagramIcon({ className = "w-4 h-4", style }: Props) {
  return (
...
export function FacebookIcon({ className = "w-4 h-4", style }: Props) {
  return (
    <svg className={className} style={style} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
...
export function LinkedinIcon({ className = "w-4 h-4", style }: Props) {
  return (
    <svg className={className} style={style} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
...
export const BRAND: Record<
  string,
  { label: string; color: string; gradient: string; Icon: (p: Props) => React.ReactElement }
> = {
  instagram: {
    label: "Instagram",
    color: "#e1306c",
  ...
  facebook: {
    label: "Facebook",
    color: "#1877f2",
    gradient: "linear-gradient(135deg,#1877f2,#0a5cd6)",
    Icon: FacebookIcon,
  },
  linkedin: {
    label: "LinkedIn",
...
/** Small rounded badge with the brand logo on its brand gradient. */
